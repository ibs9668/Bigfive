...
def domain_classfiy(uid,uid_weibo):#领域分类主函数
    '''
    用户领域分类主函数
    输入数据示例：
    ...
    timestamp = date2ts("2016-11-20") ##################需更新
    id_body = {
                                "query":{
                                    "ids":{
                                        "type":"text",
...
if __name__ == '__main__':
    ...
    count = 0
    for uid in uid_list:
        uid_weibo = get_uid_weibo(uid)
        domain,r_domain = domain_classfiy(uid,uid_weibo)
        count += 1
        print (count)
