...
from bigfive.config import es ,es_weibo
...
def cal_sentiment(uid_list):
    for i,_ in enumerate(uid_list):
        for j in ES_INDEX_LIST:
            ...
            print (sum_r)
            if sum_r != 0:
                timestamp = es_weibo.search(index=j, doc_type="text", body=query_body)["hits"]["hits"][0]["_source"]["timestamp"]
                #print (sum_r)
                #print (timestamp)
                sentiment_value = []
                for val in SENTIMENT_INDEX_LIST:
                    query_body_1 = {"query": {"bool": {"must":[{"term": {"uid": _}},{"term": {"sentiment": val}}]}}}
                ...
                #用户uid 时间 中性 积极 消极
                es.index(index = "user_emotion",doc_type = "text",id = _+ "_"+str(date2ts(ts2date(timestamp))), body = {"timestamp":date2ts(ts2date(timestamp)),"uid": _, "nuetral":  sentiment_value[0], "positive": sentiment_value[1], "negtive":sum_r-sentiment_value[0]-sentiment_value[1]})                 
            else:
                timestamp = date2ts(j.split("_")[-1])
                #print (_,timestamp,0,0,0)
                es.index(index = "user_emotion",doc_type = "text",id = _+ "_"+str(timestamp), body = {"timestamp":timestamp,"uid": _, "nuetral":  0, "positive": 0, "negtive":0})
                ...
                if result:
                    ...
                    for n in result:
                        ...
                        try:
                            ...
                            #print ("---")
                            id_es = str(target)+"_"+ str(source) +"_"+str(date2ts(ts2date(timestamp)))+"_"+str(message_type)
                            #print (id_es)
                            es.index(index = "user_social_contact",doc_type = "text",id = id_es, body = {"timestamp":date2ts(ts2date(timestamp)),"source": source, "target": target, "source_name": source_name, "target_name":target_name,"message_type":message_type})
                        except:
                            print ("no")
                        #print (source_name)